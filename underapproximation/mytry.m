...
% sdp_cons = translateSOSnew([pre_cond_ineq, range_cond], q - p_a, sdp_cons, sdeg, epsilon);

% inductive
for j = 1:branch_num
    sdp_cons = translateSOSnew([inv, inv_ineq, loop_cond, guard_cond_list(j), range_cond], replace(inv, vars, f_list(:,j))-p_a, sdp_cons, sdeg, epsilon);
end

%post
% for j = 1:length(post_cond_ineq)
%     % -loopcond
%     sdp_cons = translateSOSnew([inv, range_cond, -loop_cond], post_cond_ineq(j)-p_a, sdp_cons, sdeg, epsilon);
% end

% M
sdp_cons = translateSOSnew([range_cond], M - p_a, sdp_cons, sdeg, epsilon);
...
function sdp_cons = translateSOSnew(pre_list, post, sdp_cons, sdeg, epsilon)
    ...
    for i = 1:length(pre_list)
        [sigma_cell{tail}, sigma_coef_cell{tail}] = polynomial(vars, [sdeg,sdeg,2,2,2]);    
